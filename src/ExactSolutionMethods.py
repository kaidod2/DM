...
class MDP:
    def __init__(self,
        ...
        """
        Data structue for a Matkov Decision Process.
        MDPs are sometimes defined in terms of tuple consisting of the various component of MDP.
        :param gamma: discount factor
        :param S: state space
        :param A: action space
        :param T: transition space
        :param R: reward space
        :param TR: sample transition and reward. We will us 'TR' later to sample the next state and reward given the current state and action: s_prime , r = TR(s, a)
        """
        self.gamma = gamma
        self.S = S
        self.A = A
        ...
        if type(T) == np.ndarray:
            ...
            self.TR = lambda s, a: (np.random.choice(len(self.S), p=T[s, a]),
                                    self.R(s, a)) if not np.all(T[s, a] == 0) else (np.random.choice(len(self.S)),
                                                                                    self.R(s, a))
        else:
            self.T = T
            self.TR = TR
...
class GaussianSeidelValueIteration(ExactSolutionMethod):
    def __init__(self, k_max: int):
        self.k_max = k_max
...
class LinearQuadraticProblem:
    def __init__(self, Ts: np.ndarray,
        ...
        self.Rs = Rs  # reward matrix with respect to state (negative semidefine)
        self.Ra = Ra  # reward matrix with respect to action
    ...
    def solve(self) -> list[Callable[[np.ndarray], np.ndarray]]:
        ...
        for _ in range(1, self.h_max):
            V = (Ts.T @ (V - (V - Ta @ (np.linalg.inv(Ta.T @ V @ Ta + Ra) @ (Ta.T @ V)))) @ Ts)
